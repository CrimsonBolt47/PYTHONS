...
def search_comm():
    list1.delete(0,END)
    for row in d.search(title.get(),author.get(),year.get(),isbn.get()):
        list1.insert(END,row)

def add_comm():
    d.insert(title.get(),author.get(),year.get(),isbn.get())
    list1.delete(0,END)
    list1.insert(END,(title.get(),author.get(),year.get(),isbn.get()))

def delete_comm():
    d.delete(selected_tuple[0])

def update_comm():
    d.update(selected_tuple[0],title.get(),author.get(),year.get(),isbn.get()) 
